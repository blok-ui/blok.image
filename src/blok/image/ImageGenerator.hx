...
function process(input:String, output:String, options:ImageOptions):Task<ImageInfo> {
	...
	if (input.withoutDirectory().startsWith(thumbPrefix)) {
		return new Error(
			LoopDetected,
			'Something went wrong: the image generator is processing a temporary file it created.'
			#if debug
...
extern class Gd {
	...
	public static function imagecopyresampled(dst_image:Dynamic, src_image:Dynamic, dst_x:Int, dst_y:Int, src_x:Int, src_y:Int, dst_w:Int, dst_h:Int,
